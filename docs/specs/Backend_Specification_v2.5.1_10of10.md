...
- **Note:** `lead_source` is distinct from `channel` - it represents lead attribution (where the lead originally came from), not the communication platform (SMS/WhatsApp/Email/etc.)
...
**`reviews`** - Review records from all platforms
- **Keys:** id (uuid PK), org_id (FK), contact_id (FK nullable), job_id (FK nullable), booking_id (FK nullable)
- **Fields:** 
  - `platform` (enum: google, facebook, yelp, internal)
  - `rating` (int 1-5)
  - `review_text` (text)
  - `reviewer_name` (text)
...
- **Indexes:** (org_id, created_at), (org_id, platform, rating), (contact_id), (job_id)

**`review_requests`** - Review request tracking
- **Keys:** id (uuid PK), org_id (FK), contact_id (FK), job_id (FK nullable), booking_id (FK nullable), review_id (FK nullable)
- **Fields:**
  - `trigger_type` (enum: job_completed, payment_received, quote_accepted, scheduled, manual)
  - `status` (enum: pending, sent, received, expired)
  - `request_channel` (enum: email, sms, whatsapp, in_app)
  - `template_id` (uuid, FK to review_templates, nullable)
  - `sent_at` (timestamp, nullable)
  - `received_at` (timestamp, nullable)
  - `created_at` (timestamp)
  - `updated_at` (timestamp)
- **Indexes:** (org_id, status), (contact_id), (job_id)

**`review_responses`** - Review response history
- **Keys:** id (uuid PK), org_id (FK), review_id (FK)
- **Fields:**
  - `response_text` (text)
  - `platform` (enum: google, facebook, yelp, internal)
  - `status` (enum: draft, sent, published, failed)
  - `posted_at` (timestamp, nullable)
  - `template_id` (uuid, FK to review_templates, nullable)
  - `created_at` (timestamp)
  - `updated_at` (timestamp)
- **Indexes:** (org_id, review_id), (org_id, status)

**`review_platforms`** - Platform integration configuration
- **Keys:** id (uuid PK), org_id (FK)
- **Fields:**
  - `platform` (enum: google, facebook, yelp)
  ...
  - `created_at` (timestamp)
  - `updated_at` (timestamp)
- **Indexes:** (org_id, platform), (org_id, connected)

**`nps_surveys`** - NPS survey management
- **Keys:** id (uuid PK), org_id (FK)
- **Fields:**
  - `name` (text)
  - `question` (text)
  - `status` (enum: draft, active, paused, completed)
  - `send_channel` (enum: email, sms, whatsapp)
  - `target_audience` (jsonb, segment filters)
  - `started_at` (timestamp, nullable)
  - `ended_at` (timestamp, nullable)
  - `created_at` (timestamp)
  - `updated_at` (timestamp)
- **Indexes:** (org_id, status), (org_id, created_at)

**`nps_responses`** - NPS survey responses
- **Keys:** id (uuid PK), org_id (FK), survey_id (FK), contact_id (FK nullable)
- **Fields:**
  - `score` (int 0-10)
  ...
  - `created_at` (timestamp)
- **Indexes:** (org_id, survey_id), (org_id, category), (contact_id)

**`review_templates`** - Review request and response templates
- **Keys:** id (uuid PK), org_id (FK)
- **Fields:**
  - `template_type` (enum: request, response)
  - `name` (text)
  - `subject` (text, nullable)
  - `content` (text)
  - `platform` (enum: google, facebook, yelp, internal, all)
  - `trigger_type` (enum: job_completed, payment_received, quote_accepted, manual, nullable)
  - `is_default` (bool, default false)
  - `merge_fields` (jsonb, available fields)
  - `created_at` (timestamp)
  - `updated_at` (timestamp)
- **Indexes:** (org_id, template_type), (org_id, platform), (org_id, is_default)

**`review_analytics`** - Review analytics aggregation
- **Keys:** id (uuid PK), org_id (FK)
- **Fields:**
  - `date` (date)
  - `platform` (enum: google, facebook, yelp, internal, all)
  ...
  - `rating_1` through `rating_5` (int)
  - `response_count` (int)
  - `response_rate` (decimal)
  - `average_response_time_hours` (decimal, nullable)
  - `sentiment_positive`, `sentiment_neutral`, `sentiment_negative` (int)
  - `created_at` (timestamp)
  - `updated_at` (timestamp)
- **Indexes:** (org_id, date), (org_id, platform, date)

### Edge Functions

- `sync-reviews` - Sync reviews from connected platforms (Google, Facebook, Yelp)
- `send-review-request` - Send review request to customer via selected channel
- `post-review-response` - Post response to review platform via API
- `calculate-nps` - Calculate NPS score from survey responses
- `analyze-review-sentiment` - AI-powered sentiment analysis of review text
- `generate-review-widget` - Generate embed code for review widget display
...
#### [Enhanced Addendum — New Backend Tables & Functions: Contacts, Reviews, Notifications, Import/Export]

### Contacts ↔ Reviews
- Link reviews to contacts via `contact_id` FK
- Track review history in contact timeline
- Update contact scores based on review ratings
- Use contact segments for review request targeting

### Reviews ↔ Notifications
- Send notifications for new reviews
- Alert on low ratings (< 3 stars)
- Remind to respond to reviews
- Weekly review summary notifications
